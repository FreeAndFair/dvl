...
namespace Aegis_DVL {
    ...
    public class Station : IDisposable {
        ...
        /// </summary>
        public bool EnoughStations { [Pure] get { return Peers.Keys.Count(StationActive) >= 0; /*TODO: Correct to '>' when not testing*/ } }
        ...
        public void RequestBallot(VoterNumber voterNumber) {
            Contract.Requires(Database[voterNumber] == BallotStatus.NotReceived);
            Communicator.Send(new RequestBallotCommand(Address, voterNumber), Manager);
            if(Logger != null)
                Logger.Log("Requesting ballot for: voternumber=" + voterNumber, Level.Info);
        }
        ...
        public void BallotReceived(VoterNumber voterNumber) {
            Contract.Requires(Database[voterNumber] == BallotStatus.NotReceived);
            Contract.Ensures(Database[voterNumber] == BallotStatus.Received);
            Database[voterNumber] = BallotStatus.Received;
            if(Logger != null)
                Logger.Log("Marking voternumber=" + voterNumber + " as having received a ballot.", Level.Info);
        }
        ...
        public void RevokeBallot(VoterNumber voterNumber) {
            Contract.Requires(Database[voterNumber] == BallotStatus.Received);
            Contract.Ensures(Database[voterNumber] == BallotStatus.NotReceived);
            Database[voterNumber] = BallotStatus.NotReceived;
            if(Logger != null)
                Logger.Log("Revoking ballot for voter with voternumber=" + voterNumber, Level.Warn);
        }
        ...
        public void AnnounceRevokeBallot(VoterNumber voterNumber, CPR cpr) {
            ...
            cmd.Execute(this);
            if(Logger != null)
                Logger.Log("Announcing that this ballot should be revoked: voternumber=" + voterNumber, Level.Warn);
        }
