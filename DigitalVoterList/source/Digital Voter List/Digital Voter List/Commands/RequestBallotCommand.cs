    ...
    public class RequestBallotCommand : ICommand {
        ...
        public void Execute(Station receiver) {
            if(!receiver.IsManager)
                return;
            if(receiver.Database[_voternumber] != BallotStatus.NotReceived) {
                receiver.Communicator.Send(new BallotRequestDeniedCommand(receiver.Address), Sender);
                receiver.Logger.Log("Attempted to request a ballot that had status " + receiver.Database[_voternumber], Level.Info);
                return;
            }
            receiver.BallotReceived(_voternumber);
