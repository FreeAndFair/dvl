...
namespace Aegis_DVL.Database {
    class SqLiteDatabase : IDatabase {
        ...
        public BallotStatus this[VoterNumber voternumber] {
            get {
                var voter = GetVoter(voternumber);
                if(voter == null)
            ...
            set {
                if(Parent.Logger != null)
                    Parent.Logger.Log("Setting ballotstatus for voternumber=" + voternumber + " to " + value, Level.Info);
                var voter = GetVoter(voternumber);
        ...
        {
            var encvoterNumber = Parent.Crypto.AsymmetricEncrypt(Bytes.From(voterNumber.Value), Parent.Crypto.VoterDataEncryptionKey);

            var res = _db.Voters.Where(data => data.VoterNumber == encvoterNumber.Value);
            return !res.Any() ? null : res.Single();
        }
